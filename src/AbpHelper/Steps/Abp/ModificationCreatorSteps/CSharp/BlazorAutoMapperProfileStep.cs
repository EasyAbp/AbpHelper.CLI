...
{
    public class BlazorAutoMapperProfileStep : CSharpModificationCreatorStep
    {
        protected override IList<ModificationBuilder<CSharpSyntaxNode>> CreateModifications(WorkflowExecutionContext context, CompilationUnitSyntax rootUnit)
        {
            ...
            string usingText = TextGenerator.GenerateByTemplateName(templateDir, "BlazorAutoMapperProfile_Using", model);

            string contents = TextGenerator.GenerateByTemplateName(templateDir, "BlazorAutoMapperProfile_CreateMap", model);
        ...
        public BlazorAutoMapperProfileStep([NotNull] TextGenerator textGenerator) : base(textGenerator)
        {
        }
    }
}